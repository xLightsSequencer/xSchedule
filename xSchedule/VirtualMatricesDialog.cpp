...
VirtualMatricesDialog::VirtualMatricesDialog(wxWindow* parent, OutputManager* outputManager, std::list<VirtualMatrix*>* vmatrices, ScheduleOptions* options, wxWindowID id, const wxPoint& pos, const wxSize& size) :
    _vmatrices(vmatrices) {
...
void VirtualMatricesDialog::OnButton_OkClick(wxCommandEvent& event) {
    ...
    for (int i = 0; i < ListView1->GetItemCount(); i++) {
        ...
        if (ListView1->GetItemText(i, 2) != "") {
            width = wxAtoi(ListView1->GetItemText(i, 1));
            height = wxAtoi(ListView1->GetItemText(i, 2));
            rotation = ListView1->GetItemText(i, 4).ToStdString();
            startChannel = ListView1->GetItemText(i, 7).ToStdString();
            pixelChannels = ListView1->GetItemText(i, 10).ToStdString();
        } else {
            fromModel = ListView1->GetItemText(i, 1);
        }
        _vmatrices->push_back(new VirtualMatrix(_outputManager, width, height, topMost, rotation, pixelChannels, quality, startChannel, name, size, location, useMatrixSize, matrixMultiplier, fromModel));
    }
...
void VirtualMatricesDialog::DoAdd() {
    ...
    VirtualMatrixDialog dlg(this, _outputManager, name, rotation, pixelChannels, quality, size, location, width, height, topMost, startChannel, useMatrixSize, matrixMultiplier, _options, fromModel);
...
void VirtualMatricesDialog::DoEdit() {
    ...
    if (ListView1->GetItemText(item, 2) != "") {
        width = wxAtoi(ListView1->GetItemText(item, 1));
        height = wxAtoi(ListView1->GetItemText(item, 2));
        rotation = ListView1->GetItemText(item, 4).ToStdString();
        pixelChannels = ListView1->GetItemText(item, 10).ToStdString();
        startChannel = ListView1->GetItemText(item, 7).ToStdString();
    } else {
        fromModel = ListView1->GetItemText(item, 1);
    }

...
public:
    ArtNetTimecodeThread(SyncArtNet* syncArtNet, ScheduleManager* scheduleManager) {
        ...
        if (Run() != wxTHREAD_NO_ERROR) {
            spdlog::error("Failed to start ArtNet Timecode thread");
        } else {
            spdlog::info("ArtNet Timecode thread created.");
        }
    }
    ...
    void Stop() {
        spdlog::info("ArtNet Timecode thread stopping.");
        _stop = true;
    }
    ...
    void* Entry() {
        ...
        _running = false;
        spdlog::info("ArtNet Timecode thread stopped.");
        return nullptr;
    }
};
...
SyncArtNet::SyncArtNet(SYNCMODE sm, REMOTEMODE rm, const ScheduleOptions& options, ScheduleManager* schm, ListenerManager* listenerManager, const std::string& localIP) :
    ...
    if (sm == SYNCMODE::ARTNETMASTER) {
        ...
        if (_artnetSocket == nullptr) {
            spdlog::error("Error opening datagram for ARTNet Sync as master. {}", localaddr.IPAddress().ToStdString());
        } else if (!_artnetSocket->IsOk()) {
            spdlog::error("Error opening datagram for ARTNet Sync as master. {} OK : FALSE", localaddr.IPAddress().ToStdString());
            delete _artnetSocket;
            _artnetSocket = nullptr;
        } else if (_artnetSocket->Error()) {
            spdlog::error("Error opening datagram for ARTNet Sync as master. {} : {}",
                          _artnetSocket->LastError(),
                          DecodeIPError(_artnetSocket->LastError()));
            delete _artnetSocket;
            _artnetSocket = nullptr;
        } else {
            spdlog::info("ARTNet Sync as master datagram opened successfully.");
...
void SyncArtNet::SendSync(uint32_t frameMS, uint32_t stepLengthMS, uint32_t stepMS, uint32_t playlistMS, const std::string& fseq, const std::string& media, const std::string& step, const std::string& timeItem, uint32_t stepno, int overridetimeSecs) const {
    ...
    spdlog::debug("ArtnetSync: {:02d}:{:02d}:{:02d}.{:02d}", buffer[17], buffer[16], buffer[15], buffer[14]);

...
MatricesDialog::MatricesDialog(wxWindow* parent, OutputManager* outputManager, std::list<MatrixMapper*>* matrices, wxWindowID id, const wxPoint& pos, const wxSize& size) :
    _matrices(matrices) {
...
void MatricesDialog::OnButton_OkClick(wxCommandEvent& event) {
    ...
    for (int i = 0; i < ListView1->GetItemCount(); i++) {
        ...
        int strings = 0;
        int stringLength = 0;
        int strandsPerString = 0;
        std::string startLocation = "";
        std::string startChannel = "";
        if (ListView1->GetItemText(i, 2) == "") {
            fromModel = ListView1->GetItemText(i, 1).ToStdString();
        } else {
            orientation = ListView1->GetItemText(i, 1).ToStdString();
            strings = wxAtoi(ListView1->GetItemText(i, 2));
            stringLength = wxAtoi(ListView1->GetItemText(i, 3));
            strandsPerString = wxAtoi(ListView1->GetItemText(i, 4));
            startLocation = ListView1->GetItemText(i, 5).ToStdString();
            startChannel = ListView1->GetItemText(i, 6).ToStdString();
        }

        _matrices->push_back(new MatrixMapper(_outputManager, strings, strandsPerString, stringLength, orientation, startLocation, startChannel, name, fromModel));
    }
...
void MatricesDialog::DoAdd() {
    ...
    MatrixDialog dlg(this, _outputManager, name, orientation, startLocation, stringLength, strings, strandsPerString, startChannel, fromModel);
    ...
    if (dlg.ShowModal() == wxID_OK) {
        ...
        if (fromModel == "") {
            ...
            ListView1->SetItem(row, 1, fromModel);
            ListView1->SetItem(row, 2, "");
            ListView1->SetItem(row, 3, "");
            ListView1->SetItem(row, 4, "");
            ListView1->SetItem(row, 5, "");
            ListView1->SetItem(row, 6, "");
        }
    }
...
void MatricesDialog::DoEdit() {
    ...
    std::string startLocation = "";
    std::string startChannel = "";
    if (ListView1->GetItemText(item, 2) == "") {
        fromModel = ListView1->GetItemText(item, 1).ToStdString();
    } else {
        orientation = ListView1->GetItemText(item, 1).ToStdString();
        strings = wxAtoi(ListView1->GetItemText(item, 2));
        stringLength = wxAtoi(ListView1->GetItemText(item, 3));
        strandsPerString = wxAtoi(ListView1->GetItemText(item, 4));
        startLocation = ListView1->GetItemText(item, 5).ToStdString();
        startChannel = ListView1->GetItemText(item, 6).ToStdString();
    }
    ...
    if (dlg.ShowModal() == wxID_OK) {
        ...
        if (fromModel == "") {
            ...
            ListView1->SetItem(item, 1, fromModel);
            ListView1->SetItem(item, 2, "");
            ListView1->SetItem(item, 3, "");
            ListView1->SetItem(item, 4, "");
            ListView1->SetItem(item, 5, "");
            ListView1->SetItem(item, 6, "");
        }
    }
...
void MatricesDialog::PopulateList() {
    for (const auto& it : *_matrices) {
        ...
        if (it->GetFromModel() == "") {
            ...
            ListView1->SetItem(row, 1, it->GetFromModel());
            ListView1->SetItem(row, 2, "");
            ListView1->SetItem(row, 3, "");
            ListView1->SetItem(row, 4, "");
            ListView1->SetItem(row, 5, "");
